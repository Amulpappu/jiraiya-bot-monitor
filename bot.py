...
async def process_kit_message(message: discord.Message, cfg: dict, is_backfill: bool = False, is_full_rescan: bool = False):
    ...
    try:
        ...
        if qty["ck"] > 0:
            sheets.append_transaction_entry(ck_subtotal or total, f"{qty['ck']}x", "Cleaning Kit", employee=author_emp, message_id=str(message.id), timestamp=msg_ts)
        if qty["rk"] == 0 and qty["ck"] == 0 and total > 0:
            sheets.append_transaction_entry(total, "Kit Sale", "Repair Kit", employee=author_emp, message_id=str(message.id), timestamp=msg_ts)
    except Exception as e:
    ...
    if not message.attachments and message.content:
        ...
        if numbers:
            try:
                ...
                sheets.append_transaction_entry(upg_val, "Car UpGrade", "Car UpGrade", employee=author_emp, message_id=str(message.id), timestamp=msg_ts)
                await safe_add_reaction(message, "✅")
                return
            except Exception:
                pass
